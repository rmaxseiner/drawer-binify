...
class GridfinityBaseplate:
    ...
    def create_printable_object(self, printable_object, output_dir="generated_files"):
        ...
        try:
            ...
            fcstd_path = os.path.abspath(os.path.join(output_dir, f"{doc_name}.FCStd"))
            stl_path = os.path.abspath(os.path.join(output_dir, f"{doc_name}.stl"))
    ...
    def create_baseplate(self, drawer_width, drawer_depth, print_bed_width=180, print_bed_depth=180,
                        output_dir="generated_files"):
        """
        Create a Gridfinity baseplate for the given drawer dimensions

        Parameters:
        drawer_width (float): Width of the drawer in mm
        drawer_depth (float): Depth of the drawer in mm
        print_bed_width (float): Width of the print bed in mm
        print_bed_depth (float): Depth of the print bed in mm
        output_dir (str): Directory to save output files

        Returns:
        list: Names of created baseplate files
        """
        try:
            units = self.grid_divider()
            printable_objects = self.printable_object_selector(units)
            ...
            # Create each section and track dimensions
            for printable_object in printable_objects:
                doc_name, dimensions = self.create_printable_object(printable_object, output_dir)
                if doc_name:
                    created_files.append(doc_name)
                    section_dimensions.append(dimensions)

                    # Update total bounds
                    pos_x, pos_y = dimensions['position']
                    total_bounds['min_x'] = min(total_bounds['min_x'], pos_x)
                    total_bounds['min_y'] = min(total_bounds['min_y'], pos_y)
                    total_bounds['max_x'] = max(total_bounds['max_x'], pos_x + dimensions['width'])
                    total_bounds['max_y'] = max(total_bounds['max_y'], pos_y + dimensions['depth'])

            # Validate total dimensions
            total_width = total_bounds['max_x'] - total_bounds['min_x']
            total_depth = total_bounds['max_y'] - total_bounds['min_y']

            if abs(total_width - self.drawer_width) > 0.1 or abs(total_depth - self.drawer_depth) > 0.1:
                logger.error(f"Total baseplate dimensions mismatch:")
                logger.error(f"Expected: {self.drawer_width}x{self.drawer_depth}")
                logger.error(f"Actual: {total_width:.2f}x{total_depth:.2f}")
                raise ValueError("Final baseplate dimensions do not match specifications")

            return created_files

        except Exception as e:
            logger.error(f"Error in generate_baseplate: {e}")
            raise
